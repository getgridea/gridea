<template>
    <div>
        <a-card v-if="currentThemeConfig.length > 0" :bordered="false">
            <a-button style="margin-right: 16px;" slot="extra" @click="resetThemeCustomConfig">{{ $t('reset') }}
            </a-button>
                ...
                <a-tab-pane :tab="group" v-for="(group, index) in groups" :key="index + 1">
                    <div v-for="(item, index1) in currentThemeConfig" :key="index1">
                        <a-form-item v-if="item.group === group" :label="item.label" :colon="false" :help="item.note">
                            ...
                            <!-- 普通输入 -->
                            <a-input v-if="item.type === 'input' && !item.card" :placeholder="item.note"
                            ...
                            >
                                <color-card slot="content"
                                            @change="handleColorChange($event, index1, item.name)"></color-card>
                                <a-input :ref="`color${index1}`" v-if="item.type === 'input' && item.card === 'color'"
                                         :placeholder="item.note" v-model="form[item.name]"/>
                            </a-popover>
                            ...
                            <a-textarea v-if="item.type === 'textarea'" v-model="form[item.name]"
                                        :placeholder="item.note" :autosize="{ minRows: 2, maxRows: 32 }"/>
